import {Feature as GeojsonFeature, FeatureCollection, Geometry} from "geojson"
import Pbf from "pbf"

type Coords = [number, number][]

export class MvtToGeojson {
    ...
    private constructor(extent: number, x: number, y: number, z: number) {
    ...
    /**
     * Reads the buffer and converts all features into geojson
     * @param buffer
     * @param x
     * @param y
     * @param z
     * @param layers: the layers to convert. If not given, all layers will be returned
     */
    public static async fromBuffer(buffer: ArrayBuffer, x: number, y: number, z: number, layers?: string[]): Promise<GeojsonFeature[]> {
        ...
        for (const layer of data.layers) {
            ...
            for (const feature of layer.features) {
                const properties = MvtToGeojson.inflateProperties(feature.tags, layer.keys, layer.values)
    ...
    private static inflateProperties(tags: number[], keys: string[], values: { string_value: string }[]) {
        const properties = {}
    ...
    private static classifyRings(rings: Coords[]): Coords[][] {
        ...
        // @ts-ignore
        let currentPolygon: Coords[] = undefined
    ...
    private toGeoJson(geometry: number[], typeIndex: 1 | 2 | 3, properties: any): GeojsonFeature {
        let coords: Coords[] = this.encodeGeometry(geometry)
        let classified: any = undefined
        switch (typeIndex) {
            case 1:
                const points: any[] = []
            ...
            case 3:
                classified = MvtToGeojson.classifyRings(coords)
        ...
        let type: string = MvtToGeojson.geom_types[typeIndex]
